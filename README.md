...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
  SQL is a language that specializes in databases. A relational database is a type of database that can be created using SQL.

- [ ] Why do tables need a `primary key`?
  A primary key is used so that every item in a table has a unique reference so that information can be referenced and pulled from the database when needed.

- [ ] What is the name given to a table column that references the primary key on another table.
  foreign key

- [ ] What do we need in order to have a _many to many_ relationship between two tables.
  intermediary table linking the two primary keys
